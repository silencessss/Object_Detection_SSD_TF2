...
class MultiBoxLoss:
    def __init__(self, num_classes, overlap_thresh, neg_pos):
        self.default_boxes = tf.convert_to_tensor(DefaultBoxes().generate_boxes())  # Tensor, shape: (候選框總數(8732), 4)
    ...
    def __call__(self, y_true, y_pred):
        ...
        :param y_pred:  Tuple, (loc, conf), 其中loc的shape是(batch_size, 候選框總數, 4), conf的shape是(batch_size, 候選框總數, 21)
        :return:
        """
