...
def intersect(box_a, box_b):
    ...
    :return: box_a和box_b的交集的面積, shape: (A, B)
    """
...
def jaccard(box_a, box_b):
    ...
    :param box_b: Tensor, 預測的bounding boxes, shape: (b, 4)
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t):
    """
    匹配Ground-truth bounding-box與defaultBoxes
    :param threshold:
    :param truths:
    :param priors:
    :param variances:
...
def point_form(boxes):
    """
    將boxes的座標從(cx,cy, w, h)轉換為(xmin, ymin, xmax, ymax)格式
    :param boxes:
    :return:
    """
...
def center_size(boxes):
    """
    將boxes的座標從(xmin, ymin, xmax, ymax)轉換為(cx,cy, w, h)格式
    :param boxes:
    :return:
    """
    return tf.concat(values=[
